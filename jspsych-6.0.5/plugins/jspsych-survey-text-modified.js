...
jsPsych.plugins['survey-text-modified'] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
        ...
        for (var i = 0; i < trial.questions.length; i++) {
            ...
            if (trial.questions[i].rows == 1) {
                html += '<input id="test" type="text" name="#jspsych-survey-text-response-' + i + '" size="' + trial.questions[i].columns + '" value="' + trial.questions[i].value + '" ' + autofocus + '></input>';
            } else {
        ...
        function end() {
            ...
            var first = null
            var last = null
            if (character_times.length > 0) {
                first = character_times[0] - startTime
                last = character_times[character_times.length - 1] - startTime
            }
